...
from utils.weather_data import fetch_weather, refine_weather
...
class WeatherService:
    ...
    def check_address(self, location: str) -> Address:
        redis_key = f"checkAddress:{location}"
        cached_address = redis_service.get_cached(redis_key)
    ...
    def get_weather_hourly(self, location: str, number_of_days: int) -> WeatherHourlyResponse:
        redis_key = f"weatherHourly:{location}:{number_of_days}"
        cached_weather_hourly = redis_service.get_cached(redis_key)
    ...
    def test(self, location):
        url = f"{self.base_url}/{location}"
        params = get_params_weather(self.api_key)
        return fetch_weather(url, params)                                                     
